...
# Branding applied to every channel created to park a claimed username.
CLAIM_TITLE = "Claimed by Kro"
CLAIM_MESSAGE = (
    "KroTheCommander was here\n"
    "This User claimed by TheRulerKro\n"
    "Dev @krofullpower\n"
    "Ch @dlxfullpower"
)

# How many times to retry assigning the username on the SAME parked channel
# before giving up on this pass (it will still be retried on the next check
# cycle using the same channel, never a new one).
...
class Database:
    ...
    async def init(self, defaults: dict):
        ...
        try:
            await self._conn.execute("ALTER TABLE usernames ADD COLUMN claim_channel_id INTEGER")
            await self._conn.commit()
        except aiosqlite.OperationalError:
            pass  # column already exists (DB created by an earlier version)
    ...
    async def get_claim_channel(self, username: str) -> Optional[int]:
        cur = await self._conn.execute(
            "SELECT claim_channel_id FROM usernames WHERE username = ?", (username,)
        )
        row = await cur.fetchone()
        return row["claim_channel_id"] if row and row["claim_channel_id"] else None

    async def set_claim_channel(self, username: str, channel_id: Optional[int]):
        await self._conn.execute(
            "UPDATE usernames SET claim_channel_id = ? WHERE username = ?",
            (channel_id, username),
        )
        await self._conn.commit()
...
class MonitorEngine:
    ...
    async def _try_claim(self, uname: str):
        # Reuse the channel already created for this username on a previous
        # (failed) attempt instead of creating a fresh one every time — a new
        # channel is only ever created once per username.
        channel = await self._get_claim_channel_entity(uname)

        if channel is None:
            try:
                channel = await self._create_claim_channel(uname)
                ...
                await self.notify(f"FloodWait while creating a channel for @{uname}: wait {e.seconds} seconds.")
                if not self.floodwait_auto:
                    return
                await asyncio.sleep(e.seconds + 2)
                try:
                    channel = await self._create_claim_channel(uname)
                except Exception as e2:  # noqa: BLE001
                    await self.notify(f"Still could not create a channel for @{uname}:\n{e2}")
                    return
            except Exception as e:  # noqa: BLE001
                await self.notify(f"Failed to create a channel for @{uname}:\n{e}")
                return

        await self._attempt_username_assignment(uname, channel)

    async def _create_claim_channel(self, uname: str):
        """Create the (single) parked channel/group for this username and
        remember its id so future attempts reuse it instead of making a new one."""
        is_group = self.target_type == "group"
        result = await self.client(
            CreateChannelRequest(title=CLAIM_TITLE, about="", megagroup=is_group)
        )
        new_chat = result.chats[0]
        await self.db.set_claim_channel(uname, new_chat.id)
        return new_chat

    async def _get_claim_channel_entity(self, uname: str):
        """Return the previously-created parked channel for this username, if any."""
        channel_id = await self.db.get_claim_channel(uname)
        if channel_id is None:
            return None
        try:
            return await self.client.get_entity(PeerChannel(channel_id))
        except Exception:  # noqa: BLE001
            # The parked channel can no longer be resolved (e.g. deleted
            # manually) — forget it so a new one gets created instead of
            # retrying forever against nothing.
            await self.db.set_claim_channel(uname, None)
            return None

    async def _attempt_username_assignment(self, uname: str, channel, retries: int = MAX_ASSIGN_RETRIES):
        """
        Try to set `uname` on the already-created `channel`.

        Only a confirmed UsernameOccupiedError means someone genuinely beat
        us to it — that's the sole case where we give up on this username.
        Any other error (including Telegram's generic/ambiguous failures) is
        treated as inconclusive: we retry on this SAME channel instead of
        creating another one, since repeated channel creation is what risks
        getting the account limited/banned.
        """
        for attempt in range(retries):
            try:
                await self.client(ChannelUpdateUsername(channel=channel, username=uname))
                link = f"https://t.me/{uname}"
                await self._post_claim_message(channel)
                await self.db.mark_assigned(uname, channel.id, link)
                await self.db.set_claim_channel(uname, None)
                kind = "channel" if self.target_type == "channel" else "group"
                await self.notify(f"Claimed @{uname} — created a {kind} and assigned the username.\nLink: {link}")
                return

            except UsernameOccupiedError:
                # Confirmed by Telegram: the username is actually taken,
                # most likely someone else grabbed it a moment before us.
                ...
                await self.notify(f"FloodWait while assigning @{uname}: wait {e.seconds} seconds.")
                if not self.floodwait_auto:
                    return
                await asyncio.sleep(e.seconds + 2)
                # loop continues on the SAME channel

            except Exception as e:  # noqa: BLE001
                # Ambiguous/transient error — Telegram did NOT confirm the
                # username is occupied, so we do not treat it as lost and we
                # do not create a new channel. We just retry this channel.
                if attempt == retries - 1:
                    await self.notify(
                        f"Could not confirm @{uname} after {retries} attempts "
                        f"(last error: {e}). Keeping the same parked channel and "
                        f"will keep retrying on it on the next check — no new "
                        f"channel will be created."
                    )
                    return
                await asyncio.sleep(1.5 * (attempt + 1))

    async def _post_claim_message(self, channel):
        try:
            await self.client.send_message(channel, CLAIM_MESSAGE)
        except Exception:  # noqa: BLE001
            log.exception("Failed to post the claim message in the new channel")
